...
- `columna` 🆕 → agregada por las migraciones `0001`–`0007` para este algoritmo (**aplicadas el 2026-09-24**). Todas existen en la base.
- ⚠️ → diferencia con el borrador original, o punto a tener en cuenta al implementar.
...
- El único cálculo de precio del sistema ocurre **cuando un Transportista se postula** (RF-17). Usa su `vehiculo` real (con `vehiculo_costo` 🆕) y la `oferta.cantidad_ayudantes` que elige.
...
> ⚠️ `solicitud.cotizacion_estimada_monto` sigue existiendo (nullable), marcada **DEPRECATED** (0003). Se elimina en una migración posterior. El backend **no la escribe**.
>
> ⚠️ **Pendiente en la app:** `fletway-mobile/docs/API_CONTRATOS.md` todavía define `POST /solicitudes` → `CotizacionEstimada`. Hay que alinearlo cuando se construya ese endpoint.
...
Según cuál se elija cambia **dónde vive** el campo de configuración. Por eso el schema **no tiene ninguna columna de configuración de margen**. Lo único que existe es el **valor aplicado** en cada oferta (`oferta.margen_pct` 🆕) y congelarlo en el viaje (`viaje.margen_pct_snapshot` 🆕). Eso sirve para cualquiera de las dos opciones. En el pseudocódigo aparece como `margenPct`, sin decir de dónde se lee.
...
Sistema ejecuta (con el rol del Transportista — RLS pass-through, D-02):
  ...
  5. precioFinal()          → precio que paga el Cliente
        │
        ▼
INSERT oferta (precio_calculado + desglose 🆕)
        │
...
| `solicitud` | `pisos_origen` 🆕, `ascensor_utilizable_origen` 🆕, `distancia_vehiculo_origen_m` 🆕, `pisos_destino` 🆕, `ascensor_utilizable_destino` 🆕, `distancia_vehiculo_destino_m` 🆕 | ✅ (`requiere_escalera` y `pisos_escalera` quedan DEPRECATED) |
| `solicitud_objeto` | `cantidad`, `peso_unitario_kg`, `largo_m` 🆕, `ancho_m` 🆕, `alto_m` 🆕 | ✅ (`volumen_unitario_m3` queda DEPRECATED) |
| `oferta` (en construcción) | `vehiculo_id`, `cantidad_ayudantes` | ✅ existen |
| `vehiculo` | `peso_maximo_kg` (**es carga útil**), `largo_util_m` 🆕, `ancho_util_m` 🆕, `alto_util_m` 🆕 | ✅ (`volumen_carga_m3` queda DEPRECATED) |
| `vehiculo_costo` 🆕 (1:1, privada) | ver §4.2 | ✅ |
| Resultado de `planificarViajes()` | cantidad de viajes y carga de cada uno | → `oferta.cantidad_viajes` ✅ |
| `config_costo_laboral` 🆕 (vigente) | ver §4.1 | ✅ (1 fila seed ilustrativa) |
| `config_operacion` 🆕 (vigente) | ver §4.3 | ✅ (1 fila seed ilustrativa) |
| `config_comision` (vigente) | `porcentaje` (**0–100**, numeric(5,2)) | ✅ legible por cualquier usuario autenticado desde 0007 |
| `config_impuesto` 🆕 (vigente) | `iva_pct` (0–100) | ✅ (1 fila seed: 21 %) |
| `margenPct` | — | 🔓 **decisión abierta** (§0.4) |

> ⚠️ **Diferencias con el borrador:**
...
```go
type Acceso struct {
    Pisos              int     // solicitud.pisos_origen / pisos_destino 🆕
    AscensorUtilizable bool    // solicitud.ascensor_utilizable_origen / _destino 🆕
    DistanciaVehiculoM float64 // solicitud.distancia_vehiculo_origen_m / _destino_m 🆕
}
...
type Objeto struct {
    ...
    LargoM, AnchoM, AltoM float64 // solicitud_objeto.largo_m / ancho_m / alto_m 🆕 (alto = eje vertical)
    ...
    RotacionHorizontal bool    // solicitud_objeto.rotacion_horizontal 🆕
    RotacionVertical   bool    // solicitud_objeto.rotacion_vertical 🆕
    Apilable           bool    // solicitud_objeto.apilable 🆕
}
...
type Vehiculo struct {
    ...
    LargoUtilM, AnchoUtilM, AltoUtilM float64 // vehiculo.largo_util_m / ancho_util_m / alto_util_m 🆕
    Costo                     VehiculoCosto
}
// VolumenUtilM3() = LargoUtilM * AnchoUtilM * AltoUtilM

type VehiculoCosto struct { // vehiculo_costo 🆕 (1:1, privada)
...
> ⚠️ **Discrepancias con las estructuras del borrador:**
...
### 4.1 Costo laboral (por hora) — lee `config_costo_laboral` 🆕 vigente

```go
// Fila vigente de config_costo_laboral 🆕 (vigente_hasta IS NULL).
...
> ⚠️ **Inconsistencia interna del borrador (no resuelta):** el texto dice "alícuota de ART del empleador al **18 %**", pero la constante es `ArtPct = 0.10`. El 18 % es `ContribucionesSegSocialPct`. La seed usa **10 %**, el valor del código. Hay que confirmarlo contra la normativa vigente antes de producción.
>
...
> ⚠️ **Por qué es una tabla aparte:** `vehiculo_select` es `USING (true)` (lectura pública). Si estos campos fueran columnas de `vehiculo`, cualquier usuario vería el valor de compra y el seguro de cada Transportista. Por eso viven en **`vehiculo_costo` 🆕** (1:1, PK = `vehiculo_id`), con policies de select/insert/update sólo para el dueño y el Administrador. Decisión confirmada el 2026-09-24.
>
...
### 4.3 Duración de la operación (carga y descarga) — lee `config_operacion` 🆕 vigente
...
```go
// Fila vigente de config_operacion 🆕.
...
type DesgloseOferta struct { // se persiste en oferta 🆕 — ver §6
...
>
> ⚠️ **Ambigüedades heredadas del borrador (no resueltas, confirmar antes de implementar):**
...
// ivaPct:      config_impuesto.iva_pct 🆕 vigente (0–100).
...
- ⚠️ **Discrepancia de escala:** el borrador usa `ComisionPct = 0.15`, pero `config_comision.porcentaje` en la base es **numeric(5,2) con CHECK 0–100**, o sea 15.00. Por eso el pseudocódigo divide por 100.
...
- `iva_pct` supone 21 %. El tratamiento fiscal definitivo depende de la estructura jurídica de Fletway y del Transportista. Se parametriza en `config_impuesto` 🆕, no se hardcodea.
- 🔓 **`margenPct`: decisión abierta** (§0.4). El borrador lo dejaba "global por defecto (0.00)". Acá **no** se toma esa decisión: el valor aplicado se guarda en `oferta.margen_pct` 🆕 sin importar de dónde salga.
...
|---|---|---|
| cantidad de viajes | `cantidad_viajes` ✅ (CHECK > 0) | `cantidad_viajes_snapshot` 🆕 |
| cantidad de ayudantes | `cantidad_ayudantes` ✅ | `cantidad_ayudantes` ✅ (columna operativa) |
| distancia origen→destino | `distancia_km` 🆕 | `distancia_km_snapshot` ✅ (se reutiliza) |
| duración de ruta | `duracion_ruta_h` 🆕 | `duracion_ruta_h_snapshot` 🆕 |
| duración de operación (Σ viajes) | `duracion_operacion_h` 🆕 | `duracion_operacion_h_snapshot` 🆕 |
| costo laboral | `costo_laboral` 🆕 | `costo_laboral_snapshot` 🆕 |
| costo del vehículo | `costo_vehiculo` 🆕 | `costo_vehiculo_snapshot` 🆕 |
| costos adicionales | `costos_adicionales` 🆕 | `costos_adicionales_snapshot` 🆕 |
| costo operativo | `costo_operativo` 🆕 | `costo_operativo_snapshot` 🆕 |
| margen aplicado | `margen_pct` 🆕 | `margen_pct_snapshot` 🆕 |
| precio neto | `precio_neto` 🆕 | `precio_neto_snapshot` 🆕 |
| % comisión aplicado | `porcentaje_comision` 🆕 | `porcentaje_comision_snapshot` ✅ (se reutiliza) |
| % IVA aplicado | `iva_pct` 🆕 | `iva_pct_snapshot` 🆕 |
| **precio final** | `precio_calculado` ✅ | `monto_total_snapshot` ✅ (se reutiliza) |
...
1. Leer `config_costo_laboral` 🆕, `config_operacion` 🆕, `config_comision` y `config_impuesto` 🆕 **vigentes** (`vigente_hasta IS NULL`) una vez por cálculo. No hardcodear constantes. Los porcentajes llegan en 0–100.
2. `costoPorHoraVehiculo` y `costoPorKmVehiculo` leen `vehiculo_costo` 🆕 del `oferta.vehiculo_id`. Si el vehículo **no tiene fila** en `vehiculo_costo`, devolver un error de validación. Nunca calcular con ceros.
...
5. `precioFinal` → `oferta.precio_calculado` es el único monto que ve el Cliente. **No exponer** `costo_operativo`, `precio_neto` ni el desglose en respuestas de API al Cliente, sólo al propio Transportista y al Administrador.
   ⚠️ A nivel base, `oferta_select` y `viaje_select` **sí** dejan al Cliente leer esas columnas (lectura directa por PostgREST). Ver §8.
...
- Los valores de `config_costo_laboral`, `config_operacion` y `config_impuesto` son **ilustrativos** (los del borrador). Hay que reemplazarlos por los vigentes antes de producción. ⚠️ El borrador dice "ART al 18 %" en el texto, pero la constante es 10 %; la seed usa 10 %.
